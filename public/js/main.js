...
window.onload = async () => {
    ...
    try {
        ...
        const premium = axios.get('http://13.211.101.179:3000/premium/ispremium', { headers: { "Authorization": token } });
        const exp = axios.get(`http://13.211.101.179:3000/user/allExpence?page=${page}&items=${quantity}`, { headers: { "Authorization": token } });
...
async function saveToLocal(event) {
    ...
    const exp = await axios.post('http://13.211.101.179:3000/user/addExpence', obj, { headers: { "Authorization": token } });
    try {
...
function setValueInUi(obj, id) {
    ...
    del.onclick = () => {
        const token = localStorage.getItem('token');
        const exp = axios.delete('http://13.211.101.179:3000/user/delete/' + id + '/' + obj.amount, { headers: { "Authorization": token } });
        try {
            console.log(exp);
    ...
    edit.onclick = () => {
        ...
        if (button.innerHTML === 'Submit') {
            ...
            button.onclick = (event) => {
                ...
                console.log(newObj);
                const exp = axios.post('http://13.211.101.179:3000/user/edit', newObj, { headers: { "Authorization": token } });
                try {
                    console.log(exp);
                    expList.removeChild(li);
...
async function getProducts(page) {
    ...
    try {
        const exp = await axios.get(`http://13.211.101.179:3000/user/allExpence?page=${page}&items=${quantity}`, { headers: { "Authorization": token } });
        if (exp.data.success == true) {
...
document.getElementById('rzpButton').onclick = async function (e) {
    const token = localStorage.getItem('token');
    const response = await axios.get('http://13.211.101.179:3000/premium/premiummembership', { headers: { "Authorization": token } });
    console.log(response);
    var options = {
        "key": response.data.key_id,
        "order_id": response.data.order.id,
        "handler": async function (response) {
            await axios.post('http://13.211.101.179:3000/premium/updateTransactionStatus', {
...
async function displayForPremium() {
    ...
    document.getElementById('leaderbordShow').onclick = async function (e) {
        if (leaderBordDisplayed === false) {
            ...
            const getUser = await axios.get('http://13.211.101.179:3000/premium/showleaderbord');
            try {
    ...
    downloadexpence.onclick = async () => {
        const token = localStorage.getItem('token');
        console.log(token);
        axios.get('http://13.211.101.179:3000/user/download', { headers: { "Authorization": token } })
            .then((response) => {
                console.log(response);
